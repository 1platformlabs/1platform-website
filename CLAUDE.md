...
- **[Astro](https://astro.build/) 7.2.10** — SSR with the Node standalone adapter; one long-lived process resolves the tenant from `Host` on every request
...
- **Integrations:** `@astrojs/rss` and Astro View Transitions; sitemap/robots are request-aware routes because one build serves many tenants
- **Output:** `dist/server/entry.mjs` plus `dist/client/` assets; production runs the Node adapter behind nginx

## Development

```bash
npm run dev          # Dev server → http://localhost:4321 (hot reload)
npm run build        # Build → dist/server + dist/client (SSR output)
npm run preview      # Preview the built Node output locally
```
...
- **Sitemap:** generated per tenant by `src/pages/sitemap-0.xml.ts`; do not reintroduce the build-time `@astrojs/sitemap` integration
...
Pull-request CI runs typecheck, build, design/provider guards, functional
Playwright and the container-pinned visual comparison. `.github/workflows/prod.yml`
remains the separate `push: [main]` / manual deployment workflow; its Lighthouse
step is informative (`budgetPath: ""` — it warns to Slack, it does not block).
