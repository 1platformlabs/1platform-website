...
- **Visual contracts** now cover the current system at 1440 px and 390 px for
  both locales, alongside keyboard navigation, CTA, contrast, reduced-motion
  and JavaScript-budget checks.

### Added
- Original 1Platform editorial device and desk-object artwork under
  `src/assets/editorial/`, rendered through Astro's image pipeline.

## [Unreleased]

### Added
- **Las cinco verticales que ya estaban en producción y el sitio no nombraba.** Un comercio que despacha pedidos, cobra por enlace, cobra con tarjeta presente o le vende planes recurrentes a sus clientes ya podía hacerlo con lo desplegado, y no había forma de enterarse antes de registrarse. Ahora: `/solutions/deliveries/` y `/solutions/ads/` con sus gemelas españolas, tres bloques nuevos en `/payments-invoicing/`, cinco ítems en el catálogo (23 a 28) y tres en `/features/`. Cada afirmación traza a un endpoint vivo, y las que era fácil exagerar se escribieron contra su descripción en producción: el 503 del modo mensajería es por credencial **del inquilino** y no por falta de socio; la tarifa de un envío es una referencia informativa que la plataforma ni debita ni cobra; el remanente de una campaña vuelve **cuando el gasto reportado concilia**, que no es el instante de cerrarla; y la terminal es una solicitud que exige identificación tributaria y facturación electrónica ya activas, y que puede rechazarse.
- **El guard de proveedores aprende la familia publicitaria.** El patrón se midió antes de escribirlo, con la forma exacta de la regla: `\bmeta\b` da **24 hallazgos** sobre un árbol intacto (los `<meta>` de `BaseLayout`, la prop `meta?` de `Card`, `import.meta.glob` y el sustantivo español), pone 34 de 34 páginas `/es/` en rojo y tumba 7 de los 30 self-tests. La forma acotada da **0**. Las mensajerías no entran: la capa de reparto no mapea ninguna marca, y una lista inventada sólo se puede probar contra un tell igual de inventado.
- **`tests/provider-lists-agree.spec.ts`** — la lista de proveedores vive duplicada (una escanea el fuente, la otra el HTML construido) y el self-test es **estructuralmente incapaz** de cubrir la segunda: su `fresh_tree()` copia `src`, `scripts` y `public/fonts`, no `tests/`. Hasta ahora "las dos listas son idénticas" lo sostenía la memoria de quien editara una.
- **`tests/og-images-exist.spec.ts`** — ninguna compuerta miraba los activos de `public/`. Una página que anuncia una imagen social que el build nunca copió sirve un 404 a cada red social, en silencio y para siempre.
- **Las páginas legales declaran los tres flujos de datos que esta épica publica**: al mensajero van nombre, teléfono, dirección y coordenadas **del comprador**; a la red publicitaria, permisos de gestión sobre la página del comercio; al adquirente, documentación legal e identificación tributaria. Publicar el escaparate sin la divulgación invertía la regla que la épica dice honrar.

### Changed
- **El menú de Soluciones pasa de cinco a siete**, en el header y en el footer. Las dos nuevas van antes de Whitelabel: las cinco primeras son capacidades de comercio y las dos últimas de plataforma.
- **El ítem «Suscripciones y facturación» del catálogo describía otro producto.** Decía «controla el saldo, gestiona el uso y **factura a tus clientes**» mientras su propio destino era `/pricing/`, que es el saldo del usuario **con 1Platform**. Se corrigen sus **tres** cadenas —arreglar sólo la descripción lo dejaba auto-contradictorio— y pasa a «Saldo y consumo». La clave, el icono, el destino y la posición se conservan. La capacidad que el título prometía existe de verdad y ahora tiene su propio ítem, apuntando a donde se hace.
- **La política de cookies deja de contradecir al menú.** Decía sin calificar que no mostramos anuncios: cierto de las cookies de este sitio, y falso como afirmación sobre lo que la plataforma vende. Queda acotada a su alcance real, en los dos idiomas.
- **The host's `activate.sh` is now kept in sync by CI.** The publish job uploaded `app.zip`, its checksum and `latest.json` — and nothing else. The activator, the one thing that decides what a browser is actually served, lived only on the host and was installed by hand, so a fix to it could merge, pass every check and never run: measured on 2026-08-04, the asset grace window was inert on all three docroots the moment it merged. `deploy/cpanel/publish-activator.sh` now ships it over the same FTPS channel, before the bundle, so the release that needs the new activator is activated by it.

  The design is built around one hazard: FTP `put` does not carry the executable bit, and a mode-644 activator would stop deploys on any docroot whose cron invokes it directly — silently, on every host at once. So it stages beside the live script, chmods the staged copy, requires the **server's own listing** to say it is owner-executable, reads it back and compares sha256 — and only then does the atomic rename. Every failure path leaves the running activator exactly where it was; the worst case is "this host keeps its previous copy", never "this host cannot deploy". Verified against a real vsftpd: with `chmod_enable=NO` the publish is refused and the live activator is untouched and still runnable, and the mutation that moves the rename before the chmod turns that host's activator non-executable — which is the whole reason for the ordering.

  Guards: `publish-activator.sh --self-test` (7 cases, wired into the publish job, 4 mutations proven red — including "always say yes" and "accept any x, not just the owner's") and `deploy/cpanel/publish-activator.test.sh` (14 cases against a real FTP daemon, run by hand via `npm run test:deploy:activator`; deliberately not a CI gate — two vsftpd containers proved flaky on Docker Desktop, and a deploy gate that fails at random is worse than none).

### Fixed
- **A release made the previous one's hashed assets unreachable, breaking pages already open in a browser.** `deploy/cpanel/activate.sh` swaps the docroot symlink atomically, so the moment a release activates, everything under `_astro/` from the release before it stops being served — while any tab that already loaded a page still references those exact filenames. The symptom was measured on the dashboard, whose copy of this script is identical by design (see the script's header); the same code path produces it here. A release now carries the previous release's content-hashed files forward as **hardlinks** — same inode, so the grace window costs no extra bytes and no extra inodes against the shared cPanel quota — bounded to exactly one generation by reading the previous release's own `MANIFEST.sha256`, which CI writes before any grafting and which is therefore what keeps the tree from growing by a release forever. The graft runs *after* manifest verification, so that check keeps judging the tree CI produced, and *before* the swap, so the docroot never publishes a half-grafted release.

- **`deploy/cpanel/activate-graft.test.sh`** — the twin repo's end-to-end test, adjusted for what `assemble.sh` requires here and for Astro's `_astro/` layout. It runs the real `activate.sh` over real bundles and carries two negative controls: a bundle missing a file its own manifest declares must still be **rejected** even when the previous release happens to have a file of that exact name (proving the graft cannot launder a corrupt bundle), and a missing previous manifest must **refuse** to graft rather than guess at an unbounded one. It exists so the shared logic is guarded on both sides — a fix that lands in one twin and rots in the other is exactly the failure that script's header warns about.

### Fixed
- **`www.1platform.pro` was a full duplicate of the site, served with no canonicalisation.** Measured 2026-07-29: both hostnames answered `200` with **identical bodies** (63,119 bytes, same `<title>`) off the same cPanel docroot, while every canonical URL, the sitemap and the `hreflang` set name the apex. The serving contract (`deploy/cpanel/htaccess/landing.htaccess`) now issues a `301` from any `www.` host to the same path on the apex, preserving path and query. **No URL is removed or moved** — existing `www` links keep working and now resolve to the canonical address in one hop.
- **The build guard that was supposed to prevent a redirect loop blocked the fix instead.** `deploy/cpanel/assemble.sh` grepped for `https://` anywhere in a `RewriteRule` or `Redirect` and failed the build with "appears to force HTTPS". That regex cannot tell a **scheme** force (a genuine infinite loop under the zone's Cloudflare `flexible` mode, because the redirected request reaches this origin indistinguishable from the first) from a **host** canonicalisation (not a loop — the second request carries a different `Host` and stops matching). The guard now enforces that distinction directly: an `https://` target may take its host only from a `%1`…`%9` backreference, that backreference must be captured from `%{HTTP_HOST}`, and no `RewriteCond` may branch on the client scheme — which is where the previous check's real coverage lived. Verified against nine cases: the shipped contract and a commented-out counter-example pass; five loop forms, a backreference not taken from the host, and `%0` all still fail the build.
